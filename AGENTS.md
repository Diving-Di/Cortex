...
- 新业务接口使用 `/api/v1`；现有认证、聊天和轻日记兼容路径须保持兼容。
...
- 整理与报告必须先生成草稿、后由确认接口写入；报告和回忆必须校验并保存当前租户的来源。无来源报告返回 `REPORT_NO_SOURCES`，无证据回忆返回 `MEMORY_NO_EVIDENCE`。
...
- 发布前确认 Go 源码已经过 `gofmt`，并确认 Compose 的 `db`、`llm-gateway`、`backend` healthy。新 PostgreSQL 空库须通过 18 张表、RLS、ready、注册和登录验收。
