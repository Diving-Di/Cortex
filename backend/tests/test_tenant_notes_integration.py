...
def test_personal_tenant_notes_revisions_and_rls() -> None:
    ...
    try:
        ...
        assert client.post("/api/v1/tenant", headers=credentials[0], json={"name": "second"}).status_code == 405
        ...
        for note_type in ("normal", "daily", "weekly", "monthly"):
            ...
            if note_type != "normal": body["note_date"] = "2026-07-22"
            response = client.post("/api/v1/notes", headers=credentials[0], json=body)
            assert response.status_code == 201, response.text
            created.append(response.json())
        duplicate = client.post("/api/v1/notes", headers=credentials[0], json={"type": "daily", "title": "duplicate", "content": "x", "note_date": "2026-07-22"})
        assert duplicate.status_code == 409
        injected = client.post("/api/v1/notes", headers=credentials[1], json={"type": "normal", "title": "injected", "tenant_id": tenants[0]["id"]})
        assert injected.status_code == 422

        note_id = created[0]["id"]
        assert client.get(f"/api/v1/notes/{note_id}", headers=credentials[1]).status_code == 404
        assert client.patch(f"/api/v1/notes/{note_id}", headers=credentials[0], json={"content": "第二版"}).status_code == 200
        revisions = client.get(f"/api/v1/notes/{note_id}/revisions", headers=credentials[0]).json()
        assert revisions[0]["content"] == "第一版"
        restored = client.post(f"/api/v1/notes/{note_id}/revisions/{revisions[0]['id']}/restore", headers=credentials[0])
        ...
        with SessionLocal.begin() as db:
            ...
            db.execute(__import__('sqlalchemy').text("SELECT set_config('app.current_tenant_id', :id, true)"), {"id": str(tenant_id)})
        ...
        with SessionLocal.begin() as db:
            ids = [row[0] for row in db.query(User.id).filter(User.username.like(f"notes_{suffix}_%")).all()]
            if ids:
