...
def get_tenant_context(
    ...
    tenant = (
        db.query(Tenant)
        .filter(Tenant.user_id == user.id, Tenant.status == "active", Tenant.deleted_at.is_(None))
        .one_or_none()
    )
    if tenant is None:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Personal tenant is unavailable."
        )
