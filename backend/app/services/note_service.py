...
def get_note(db: Session, context: TenantContext, note_id: int) -> Note:
    note = db.query(Note).filter(Note.tenant_id == context.tenant_id, Note.id == note_id, Note.deleted_at.is_(None)).one_or_none()
    if note is None:
...
def create_note(db: Session, context: TenantContext, payload: NoteCreate) -> Note:
    ...
    count = db.query(func.count(Note.id)).filter(Note.tenant_id == context.tenant_id, Note.deleted_at.is_(None)).scalar() or 0
    if count >= tenant.note_quota:
        raise AppError("NOTE_QUOTA_EXCEEDED", "笔记数量已达到配额", 409)
    note = Note(tenant_id=context.tenant_id, created_by=context.user_id, updated_by=context.user_id, type=payload.type, title=payload.title.strip(), content=payload.content, note_date=_period_start(payload.type, payload.note_date), summary=payload.summary, word_count=_word_count(payload.content))
    if not note.title:
...
def update_note(db: Session, context: TenantContext, note_id: int, payload: NoteUpdate) -> Note:
    ...
    if "content" in values and values["content"] != note.content:
        db.add(NoteRevision(tenant_id=context.tenant_id, note_id=note.id, created_by=context.user_id, content=note.content, reason="update"))
...
def restore_revision(db: Session, context: TenantContext, note_id: int, revision_id: int) -> Note:
    note = get_note(db, context, note_id)
    revision = db.query(NoteRevision).filter(NoteRevision.tenant_id == context.tenant_id, NoteRevision.note_id == note.id, NoteRevision.id == revision_id).one_or_none()
    if revision is None:
        raise AppError("REVISION_NOT_FOUND", "历史版本不存在", 404)
    db.add(NoteRevision(tenant_id=context.tenant_id, note_id=note.id, created_by=context.user_id, content=note.content, reason="before_restore"))
