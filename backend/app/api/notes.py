...
def post_note(payload: NoteCreate, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> Note:
    try:
...
def read_note(note_id: int, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> Note:
    return get_note(db, context, note_id)

@router.patch("/{note_id}", response_model=NoteOut)
def patch_note(note_id: int, payload: NoteUpdate, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> Note:
...
def remove_note(note_id: int, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> Response:
    delete_note(db, context, note_id)
    return Response(status_code=204)

@router.get("/{note_id}/revisions", response_model=list[RevisionOut])
def revisions(note_id: int, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> list[NoteRevision]:
    get_note(db, context, note_id)
    return db.query(NoteRevision).filter(NoteRevision.tenant_id == context.tenant_id, NoteRevision.note_id == note_id).order_by(NoteRevision.created_at.desc()).all()

@router.post("/{note_id}/revisions/{revision_id}/restore", response_model=NoteOut)
def restore(note_id: int, revision_id: int, db: Session = Depends(get_db), context: TenantContext = Depends(get_tenant_context)) -> Note:
