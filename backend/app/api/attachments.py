...
async def upload_attachment(
    ...
    return await save_upload(db, context, note_id, file)


def _get(db: Session, context: TenantContext, attachment_id: int) -> Attachment:
    item = (
        db.query(Attachment)
        .filter(Attachment.tenant_id == context.tenant_id, Attachment.id == attachment_id)
        .one_or_none()
    )
    if item is None:
...
def list_attachments(
    ...
    return (
        db.query(Attachment)
        .filter(Attachment.tenant_id == context.tenant_id, Attachment.note_id == note_id)
        .all()
    )
...
def download_attachment(
    ...
    item = _get(db, context, attachment_id)
    path = safe_attachment_path(item.stored_path)
    if not path.is_file():
        raise AppError("ATTACHMENT_FILE_MISSING", "附件文件缺失", 410)
