...
def update_tenant(
    ...
    return TenantOut(
        ...
        note_quota=tenant.note_quota,
        note_count=note_count,
        attachment_quota_bytes=tenant.attachment_quota_bytes,
        ai_token_quota=tenant.ai_token_quota,
...
def restore_tenant(
    ...
    tenant = (
        db.query(Tenant).filter(Tenant.user_id == user.id, Tenant.status == "deleted").one_or_none()
    )
    if tenant is None:
        from ..core.exceptions import AppError
    ...
    note_count = (
        db.query(func.count(Note.id))
        .filter(Note.tenant_id == tenant.id, Note.deleted_at.is_(None))
        .scalar()
        or 0
    )
    tokens = (
        ...
        .filter(AiUsageRecord.tenant_id == tenant.id)
        .scalar()
        or 0
    )
    return TenantOut(
