...
def upgrade() -> None:
    ...
    op.create_index("uq_tags_tenant_name", "tags", ["tenant_id", "name"], unique=True)
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("tenant_id", "note_id", "tag_id"),
    ...
    op.create_table(
        ...
        sa.Column("stored_path", sa.String(500), unique=True, nullable=False),
        ...
        sa.ForeignKeyConstraint(
            ["tenant_id", "uploaded_by"], ["tenants.id", "tenants.user_id"], ondelete="CASCADE"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column("capabilities", sa.String(255), server_default="chat,stream", nullable=False),
    ...
    for table in ("tags", "note_tags", "attachments", "ai_providers"):
        _rls(table)


def downgrade() -> None:
    op.drop_index("ix_notes_content_trgm", table_name="notes")
    op.drop_index("ix_notes_title_trgm", table_name="notes")
    for col in ("error_code", "status", "duration_ms", "model"):
        op.drop_column("ai_usage_records", col)
    for table in ("ai_providers", "attachments", "note_tags", "tags"):
        op.drop_table(table)
