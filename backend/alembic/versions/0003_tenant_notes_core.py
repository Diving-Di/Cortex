...
def upgrade() -> None:
    ...
    op.add_column(
        "tenants", sa.Column("note_quota", sa.Integer(), server_default="10000", nullable=False)
    )
    op.add_column(
        "tenants",
        sa.Column(
    ...
    op.add_column(
        "tenants",
        sa.Column("ai_token_quota", sa.BigInteger(), server_default="1000000", nullable=False),
    )
    op.add_column("tenants", sa.Column("deleted_at", sa.DateTime(timezone=True)))
    op.execute(
        "INSERT INTO tenants (id, user_id, name) SELECT gen_random_uuid(), u.id, u.username || ' 的个人空间' FROM users u LEFT JOIN tenants t ON t.user_id=u.id WHERE t.id IS NULL"
    )
    ...
    op.execute(
        "UPDATE messages m SET tenant_id=c.tenant_id FROM conversations c WHERE c.id=m.conversation_id"
    )
    op.execute("DELETE FROM messages WHERE tenant_id IS NULL")
    op.alter_column("messages", "tenant_id", nullable=False)
    op.create_foreign_key(
        "fk_messages_tenant", "messages", "tenants", ["tenant_id"], ["id"], ondelete="CASCADE"
    )
    ...
    op.create_check_constraint(
        "ck_notes_type", "notes", "type IN ('normal','daily','weekly','monthly')"
    )
    op.create_index(
        "uq_notes_tenant_period",
        "notes",
        ["tenant_id", "type", "note_date"],
        unique=True,
        postgresql_where=sa.text("type IN ('daily','weekly','monthly') AND deleted_at IS NULL"),
    )
    op.create_unique_constraint("uq_notes_tenant_id_id", "notes", ["tenant_id", "id"])
    op.create_foreign_key(
        "fk_notes_tenant_creator",
        "notes",
        "tenants",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["tenant_id", "note_id"],
            ["notes.tenant_id", "notes.id"],
            ondelete="CASCADE",
            name="fk_revisions_tenant_note",
        ),
        sa.ForeignKeyConstraint(
            ["tenant_id", "created_by"],
            ["tenants.id", "tenants.user_id"],
            ondelete="CASCADE",
            name="fk_revisions_tenant_user",
        ),
    )
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(
        "uq_notes_tenant_period", "notes", ["tenant_id", "type", "note_date"]
    )
