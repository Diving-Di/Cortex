...
func (s *Server) organizeAI(w http.ResponseWriter, r *http.Request) {
    ...
    events, err := s.aiWorkflow().Organize(r.Context(), request.Content, request.Style)
    if err != nil {
...
func (s *Server) generateReport(w http.ResponseWriter, r *http.Request) {
    ...
    events, err := s.aiWorkflow().GenerateReport(r.Context(), prompt)
    if err != nil {
        if err.Error() == "AI_NOT_CONFIGURED" {
            httpx.WriteError(w, s.logger, apierror.New("AI_NOT_CONFIGURED", "AI 未配置，笔记功能仍可正常使用", 503))
        } else {
...
func (s *Server) memoryChat(w http.ResponseWriter, r *http.Request) {
    ...
    prompt := "仅依据给定笔记回答问题，引用格式为 [#笔记ID]；证据不足就明确说没有找到记录。\n问题：" +
        request.Question + "\n笔记：\n" + material.String()
    events, err := s.aiWorkflow().AnswerMemory(r.Context(), prompt)
    if err != nil {
        if err.Error() == "AI_NOT_CONFIGURED" {
            httpx.WriteError(w, s.logger, apierror.New("AI_NOT_CONFIGURED", "AI 未配置，笔记功能仍可正常使用", 503))
        } else {
