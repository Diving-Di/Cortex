...
func New(cfg config.Config, db *store.Store, logger *slog.Logger, version string) http.Handler {
	...
	authenticated.Use(s.authenticate())
	{
		...
		active.Use(s.requireActiveTenant())
		{
			...
			active.GET("/api/v1/growth-memories", gin.WrapF(s.listGrowthMemories))
			active.POST("/api/v1/growth-memories", gin.WrapF(s.createGrowthMemory))
			active.PATCH("/api/v1/growth-memories/:memoryID", gin.WrapF(s.updateGrowthMemory))
			active.DELETE("/api/v1/growth-memories/:memoryID", gin.WrapF(s.deleteGrowthMemory))
			active.GET("/api/v1/settings/memories", gin.WrapF(s.getMemorySettings))
			active.PUT("/api/v1/settings/memories", gin.WrapF(s.saveMemorySettings))
			active.POST("/api/v1/growth-memory-drafts", gin.WrapF(s.createMemoryDraft))
			active.POST("/api/v1/growth-memory-drafts/:draftID/confirm", gin.WrapF(s.confirmMemoryDraft))
			active.POST("/api/v1/growth-memory-drafts/:draftID/reject", gin.WrapF(s.rejectMemoryDraft))
