...
| OK-04 | 对话搜索和筛选 | ✅ 已实现 | 支持标题、消息正文和来源范围检索 | P1 | 第一阶段 |
| OK-05 | 对话重命名 | ✅ 已实现 | 支持乐观冲突保护的标题修改 | P1 | 第一阶段 |
| OK-06 | 长对话摘要与上下文压缩 | ✅ 已实现 | 摘要加最近消息，同时每轮重新检索来源 | P2 | 第二阶段 |
| OK-07 | 结构化成长记忆 | ✅ 已实现 | 可管理、可追溯、经用户确认的个人记忆 | P1 | 第三阶段 |
| OK-08 | AI 记忆建议与确认 | ✅ 已实现 | AI 只生成建议，确认后写入 | P1 | 第三阶段 |
| OK-09 | 记忆提取策略设置 | ✅ 已实现 | 类别、阈值、排除标签和保留策略 | P2 | 第四阶段 |
| OK-11 | 对话消息数和 token 用量展示 | 🟡 部分实现 | 会话级聚合并展示 | P3 | 第二阶段 |
| OK-12 | 通用 Agent 工具和网页搜索 | ⏸️ 暂缓 | 不纳入当前产品范围 | - | 不实施 |
| OK-13 | Python Markdown 文档解析服务 | ✅ 已实现 | TXT、DOCX、PDF 统一转换为 Markdown | P1 | 基础设施阶段 |
