...
- Nginx 提供 SPA 静态资源，将 `/api/` 同源反向代理到 FastAPI。
- FastAPI 启动前执行 `alembic upgrade head`。
...
| 项目 | 结果 | 证据摘要 |
| --- | --- | --- |
| 后端全量测试 | 通过 | 独立 PostgreSQL 16 空库执行 Alembic 全量迁移后，17 项测试全部通过且无跳过 |
...
| 项目 | 结果 | 证据摘要 |
| --- | --- | --- |
| 容器健康 | 通过 | PostgreSQL 与 FastAPI 健康检查通过，Nginx 前端入口返回 200 |
...
| 服务重启 | 通过 | 重启 FastAPI 容器并恢复健康后，重启前创建的笔记仍可读取 |
