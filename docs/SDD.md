...
- `cortex-full-backup-v1` 使用固定表白名单保存租户逻辑数据和私有文件，排除 Token、密钥、
  Provider、usage、敏感审计和小红书授权状态。恢复仅允许空租户，逐表重映射 ID、校验 ZIP
  路径及 SHA-256，并将知识文件重新排队索引。
- 应用级备份不能代替数据库和文件卷的基础设施灾备。
...
- AI 不能自动创建、修改或删除知识文件。
- 管理连接不能执行普通 RAG 查询。
- 研究原图、OCR 文本和来源正文随租户资源保留；用户删除来源时立即移除关联原图和已保存
  知识原文件，并软删除来源元数据以保留最小审计轨迹，后续不得再参与检索或备份。
- 小红书 Cookie 密文不进入备份；撤销授权时删除。二维码和 Chromium 临时 profile 仅在授权
  尝试 TTL 内存在，由授权清理任务删除，不作为静态文件提供。
- 普通日志不得记录第三方 URL、页面 HTML、OCR 全文、AI 完整输入输出、Cookie、nonce 或
  二维码文件路径。
